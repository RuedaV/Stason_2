classdef GJR_Const_StD < GJR_BaseModel
    properties
        mu, mu0, delta, delta0
    end

    methods (Access = public)
        function obj = GJR_Const_StD(mu0, delta0, omega0, alpha0, beta0, gamma0) % конструктор
            %obj.data   = data;
    ...
    methods (Access = protected)
        ...
        function log_ret = LogL (self)
            ...
            log_ret = sum( -0.5*(log(2*pi) + log(h2(:,1)) +...
                (self.data(:,1) - self.mu - self.delta*h2(:,1).^0.5).^2./h2(:,1)));
        end
