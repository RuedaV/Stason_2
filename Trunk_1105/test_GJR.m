...
%     a.MyDisplay();
    ...
    VaR_b = VaR_b + VaR_exceeded_b;
%     b.MyDisplay();
    omega(i-err,1) = (a.omega - b.omega)/omega0;
    alpha(i-err,1) = (a.alpha - b.alpha)/alpha0;
    beta(i-err,1)  = (a.beta  - b.beta)/beta0;
    gamma(i-err,1) = (a.gamma - b.gamma)/gamma0;
    loss(i-err,1)  = (loss_a - loss_b)/loss_a;
    loss2(i-err,1)  = (loss2_a - loss2_b)/loss2_a;
    i = i + 1;
    err = 0;
    catch
        err = err + 1;
...
while i < S
    i
%     try 
    a  = GJR_Const_StD(mu0, delta0, omega0, alpha0, beta0, gamma0);
    b  = GJR_Const(mu0, omega0, alpha0, beta0, gamma0);
    Data = a.Simulate(T);    
    a.data = Data(1:end-1);
    a.data_plus = Data;    
